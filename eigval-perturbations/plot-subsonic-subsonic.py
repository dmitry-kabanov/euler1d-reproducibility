...
OUTDIR = os.path.join('_output', 'subsonic-subsonic')
...
def plot_profiles_grouped_by_Q(vs_lamda=False):
    ...
    if vs_x:
        outfile = 'eigval-perturbations-sub-sub.pdf'
    else:
        outfile = 'eigval-perturbations-sub-sub-vs-znd-lambda.pdf'

    savefig(outfile)


if len(sys.argv) > 1:
    plot_profiles_grouped_by_Q(vs_lamda=True)
else:
    plot_profiles_grouped_by_Q(vs_lamda=False)
